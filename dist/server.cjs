...
var loginUser = async (payload) => {
  ...
  const token = import_jsonwebtoken.default.sign(tokenPayload, env_default.secret, { expiresIn: "1d" });
...
var auth = (...roles) => {
  return async (req, res, next) => {
    try {
      ...
      const decoded = import_jsonwebtoken2.default.verify(token, env_default.secret);
